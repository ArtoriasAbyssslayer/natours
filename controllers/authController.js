...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  createSendToken(
    ...
    201,
    res,
    `User created! Verification email sent! Please verify your email to continue!`,
  );
  next();
});
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 5) Check if user has verified his email address
  if (!currentUser.confirmedEmail) {
    req.body.email = currentUser.email;
    await exports.verifyEmail(req, res, next);
    return next(
      new AppError(
        'Email not verified. Please verify to continue. New email sent!',
        403,
      ),
    );
  }
...
// Classic function with kwargs returns middleware function
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  const verifyURL = `${req.protocol}://${req.get('host')}/api/v1/users/emailVerification/${token}`;

  try {
    await new Email(user, verifyURL).sendVerifictaionEmail();
    res.status(200).json({
      status: 'success',
...
exports.verifyEmailToken = catchAsync(async (req, res, next) => {
  ...
  // Find the user and update confirmedEmail
  ...
  res.status(200).json({
    ...
    redirectTo: `${req.protocol}://${req.get('host')}/me`,
  });
});
